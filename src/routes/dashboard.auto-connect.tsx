          ...
          <main className="rounded-lg border border-border bg-card p-6">
            {tab === "wizard" && <>
              ...
              {step === 4 && plan && (
                <MigrationsStep
                  plan={plan}
                  extraPreambleSql={analyze?.backend.extraPreambleSql ?? []}
                  db={db}
                  setDb={setDb}
...
function MigrationsStep({ plan, extraPreambleSql, db, setDb, onValidateDb, ack, setAck, ackTyped, setAckTyped, onNext, busy, verify }: {
  ...
  const tables = plan.tables.map((t) => ({ name: t.name, columns: t.columns, timestamps: true }));
  const rawSql = useMemo(() => buildMigrationBundle(tables, extraPreambleSql), [tables, extraPreambleSql]);
