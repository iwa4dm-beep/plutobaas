...
const MYSQL_TO_PG_TYPES: Array<[RegExp, string]> = [
  ...
  [/\btinyint(\s*\([^)]*\))?\s+unsigned\b/gi, 'smallint'],
  [/\btinyint(\s*\([^)]*\))?/gi, 'smallint'],
  [/\bsmallint(\s*\([^)]*\))?\s+unsigned\b/gi, 'integer'],
  [/\bmediumint(\s*\([^)]*\))?\s+unsigned\b/gi, 'integer'],
  [/\bmediumint(\s*\([^)]*\))?/gi, 'integer'],
  [/\bint(\s*\([^)]*\))?\s+unsigned\b/gi, 'bigint'],
  [/\bint(\s*\([^)]*\))?/gi, 'integer'],
  [/\bbigint(\s*\([^)]*\))?\s+unsigned\b/gi, 'numeric(20,0)'],
  [/\bbigint(\s*\([^)]*\))?/gi, 'bigint'],
  [/\bdouble(\s+precision)?(\s*\([^)]*\))?/gi, 'double precision'],
  [/\bfloat(\s*\([^)]*\))?/gi, 'real'],
  [/\bdatetime(\s*\([^)]*\))?/gi, 'timestamptz'],
  [/\btimestamp(\s*\([^)]*\))?/gi, 'timestamptz'],
  [/\blongtext\b/gi, 'text'],
...
export function convertMysqlStatement(input: string): string | null {
  ...
  // Unsigned columns handled by type-map above; strip any leftover UNSIGNED
  ...
  // Only touch backslash-quote pairs; naive but safe on dump output.
  s = s.replace(/\\'/g, "''");
